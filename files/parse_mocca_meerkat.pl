...
for my $subj_id (keys %$subj_hash){
    ...
    my $snapshots = &parse_mocca($sam_id, 0, $inter_fn, $worksheet);
    my $rv = &parse_mocca($sam_id, 1, $intra_fn, $worksheet);
    push @$snapshots, @$rv;
...
sub parse_mocca{
    ...
    while(<MOCCA>){
        ...
        next if !defined $orig || $meerkat ne "orig";
        
        
        my $ext = 200;
        my $ext2 = 500; # for indel
